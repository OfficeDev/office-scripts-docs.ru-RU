...
В этой статье будет поучено, как поддерживать и обновлять скрипты, которые используют API async старшей модели. Эти API имеют те же основные функции, что и теперь стандартные, синхронные API Office скриптов, но они требуют, чтобы ваш скрипт контролировал синхронизацию данных между сценарием и книгой.
...
5. Некоторые методы были переименованы для ясности и добавлены для удобства. Дополнительные сведения Office [ссылку на API](/javascript/api/office-scripts/overview) скриптов.

## <a name="office-scripts-async-api-reference-documentation"></a>Office Справочная документация по API скриптов async

API async эквивалентны API, используемым в Office надстроек. Эталонная документация находится в [разделе Excel ссылки Office API Надстройки JavaScript.](/javascript/api/excel?view=excel-js-online&preserve-view=true)
