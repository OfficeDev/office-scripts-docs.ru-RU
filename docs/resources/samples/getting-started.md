...
### <a name="access"></a><span data-ttu-id="c002b-108">Access</span><span class="sxs-lookup"><span data-stu-id="c002b-108">Access</span></span>
...
* [<span data-ttu-id="c002b-124">macOS</span><span class="sxs-lookup"><span data-stu-id="c002b-124">macOS</span></span>](https://code.visualstudio.com/shortcuts/keyboard-shortcuts-macos.pdf)
...
## <a name="gentle-introduction-to-scripting"></a><span data-ttu-id="c002b-143">Мягкое введение в сценарии</span><span class="sxs-lookup"><span data-stu-id="c002b-143">Gentle introduction to scripting</span></span>
...
<span data-ttu-id="c002b-158">[![Основы скриптов Office](../../images/getting-started-v_script.png)](https://youtu.be/8Zsrc1uaiiU "Основы скриптов")</span><span class="sxs-lookup"><span data-stu-id="c002b-158">[![Basics of Office Scripts](../../images/getting-started-v_script.png)](https://youtu.be/8Zsrc1uaiiU "Basics of Scripts")</span></span>
...
<span data-ttu-id="c002b-181">Итак, в сводке:</span><span class="sxs-lookup"><span data-stu-id="c002b-181">So, in summary:</span></span>
...
<span data-ttu-id="c002b-195">Вы можете использовать функции для организации и повторного использования кода в скрипте.</span><span class="sxs-lookup"><span data-stu-id="c002b-195">You can use functions to organize and reuse code within your script.</span></span>
...
* <span data-ttu-id="c002b-209">Как объекты, так и свойства доступны с помощью (точка) нотации, а объект слева и свойства или метода `.` `.` на правой стороне.</span><span class="sxs-lookup"><span data-stu-id="c002b-209">Both objects and properties are accessed using `.` (dot) notation, with the object on the left side of the `.` and the property or method on the right side.</span></span> <span data-ttu-id="c002b-210">Примеры: `hyperlink.address` , `range.getAddress()` .</span><span class="sxs-lookup"><span data-stu-id="c002b-210">Examples: `hyperlink.address`, `range.getAddress()`.</span></span>
...
<span data-ttu-id="c002b-226">Рассмотрим несколько методов на `workbook` объекте.</span><span class="sxs-lookup"><span data-stu-id="c002b-226">Let's look at a few methods on the `workbook` object.</span></span>
...
<span data-ttu-id="c002b-235">В этом примере:</span><span class="sxs-lookup"><span data-stu-id="c002b-235">In this example:</span></span>
...
#### <a name="method-chaining"></a><span data-ttu-id="c002b-250">Цепочка метода</span><span class="sxs-lookup"><span data-stu-id="c002b-250">Method chaining</span></span>
...
<span data-ttu-id="c002b-271">Кроме того, для удаления объекта, который может существовать или не существует, используйте следующий шаблон.</span><span class="sxs-lookup"><span data-stu-id="c002b-271">Alternatively, for deleting an object that may or may not exist, use the following pattern.</span></span>
...
* [<span data-ttu-id="c002b-282">`for` или `while`</span><span class="sxs-lookup"><span data-stu-id="c002b-282">`for` or `while`</span></span>](https://developer.mozilla.org/docs/Web/JavaScript/Guide/Loops_and_iteration)
...
<span data-ttu-id="c002b-290">Примеры:</span><span class="sxs-lookup"><span data-stu-id="c002b-290">Examples:</span></span>
...
<span data-ttu-id="c002b-306">**_Типы родного языка_**</span><span class="sxs-lookup"><span data-stu-id="c002b-306">**_Native language types_**</span></span>
...
<span data-ttu-id="c002b-321">Вы можете утверждать тип с помощью ключевого слова `as` или с помощью угловой скобки, как показано в следующем коде.</span><span class="sxs-lookup"><span data-stu-id="c002b-321">You can assert the type using the `as` keyword or using angle brackets as shown in following code.</span></span>
...
### <a name="do-pre-checks-at-the-beginning-of-the-script"></a><span data-ttu-id="c002b-372">Предварительные проверки в начале сценария</span><span class="sxs-lookup"><span data-stu-id="c002b-372">Do pre-checks at the beginning of the script</span></span>
...
<span data-ttu-id="c002b-394">Рассмотрим фрагмент следующего примера.</span><span class="sxs-lookup"><span data-stu-id="c002b-394">Consider the following example snippet.</span></span>
...
<span data-ttu-id="c002b-397">Другой сценарий — когда основная функция вызывает другую функцию, которая, в свою очередь, вызывает другую функцию (и так далее.), а вызов API, о котором вы заботитесь, происходит в нижней функции.</span><span class="sxs-lookup"><span data-stu-id="c002b-397">Another scenario is when main function calls another function, which in turn calls another function (and so on..), and the API call that you care about happens down in the bottom function.</span></span> <span data-ttu-id="c002b-398">Распространение ошибки в любом случае может `main` оказаться нецелесообразным или удобным.</span><span class="sxs-lookup"><span data-stu-id="c002b-398">Propagating the error all the way up to `main` may not be feasible or convenient.</span></span> <span data-ttu-id="c002b-399">В этом случае наиболее удобно бросать ошибку в нижнюю функцию.</span><span class="sxs-lookup"><span data-stu-id="c002b-399">In that case, throwing an error in the bottom function will be most convenient.</span></span>
...
<span data-ttu-id="c002b-400">*Предупреждение.* `try..catch` Использование внутри цикла притормозит сценарий.</span><span class="sxs-lookup"><span data-stu-id="c002b-400">*Warning*: Using `try..catch` inside of a loop will slow down your script.</span></span> <span data-ttu-id="c002b-401">Избегайте использования этого внутри или вокруг циклов.</span><span class="sxs-lookup"><span data-stu-id="c002b-401">Avoid using this inside of or around loops.</span></span>

## <a name="range-basics"></a><span data-ttu-id="c002b-402">Основы диапазонов</span><span class="sxs-lookup"><span data-stu-id="c002b-402">Range basics</span></span>

<span data-ttu-id="c002b-403">Ознакомьтесь [с базами диапазона](range-basics.md) перед дальнейшим путешествием.</span><span class="sxs-lookup"><span data-stu-id="c002b-403">Check out [Range Basics](range-basics.md) before you go further on your journey.</span></span>

## <a name="basic-performance-considerations"></a><span data-ttu-id="c002b-404">Основные соображения производительности</span><span class="sxs-lookup"><span data-stu-id="c002b-404">Basic performance considerations</span></span>

### <a name="avoid-slow-operations-in-the-loop"></a><span data-ttu-id="c002b-405">Избегайте медленных операций в цикле</span><span class="sxs-lookup"><span data-stu-id="c002b-405">Avoid slow operations in the loop</span></span>

<span data-ttu-id="c002b-406">Некоторые операции, когда они делаются внутри или вокруг таких циклов, как `for` , , , и `for..of` `map` `forEach` т.д. может привести к медленной производительности.</span><span class="sxs-lookup"><span data-stu-id="c002b-406">Certain operations when done inside/around the loop statements such as `for`, `for..of`, `map`, `forEach`, etc. can lead to slow performance.</span></span> <span data-ttu-id="c002b-407">Избегайте следующих категорий API.</span><span class="sxs-lookup"><span data-stu-id="c002b-407">Avoid the following API categories.</span></span>

* <span data-ttu-id="c002b-408">`get*` API</span><span class="sxs-lookup"><span data-stu-id="c002b-408">`get*` APIs</span></span>

<span data-ttu-id="c002b-409">Ознакомьтесь со всеми данными, которые необходимы за пределами цикла, а не чтением их внутри цикла.</span><span class="sxs-lookup"><span data-stu-id="c002b-409">Read all the data you need outside of the loop rather than reading it inside of the loop.</span></span> <span data-ttu-id="c002b-410">Иногда трудно избежать чтения внутри циклов; в этом случае убедитесь, что количество циклов не слишком большое или управление ими в пакетах, чтобы избежать необходимости цикла через большую структуру данных.</span><span class="sxs-lookup"><span data-stu-id="c002b-410">At times, it is hard to avoid reading inside of loops; in such a case, make sure your loop counts are not too large or manage them in batches to avoid having to loop through a large data structure.</span></span>

<span data-ttu-id="c002b-411">**Примечание.** Если диапазон/данные, с которые вы имеете дело, достаточно велик (скажем, >ячейки 100K), вам может потребоваться использовать расширенные методы, такие как разрыв чтения и записи на несколько фрагментов.</span><span class="sxs-lookup"><span data-stu-id="c002b-411">**Note**: If the range/data you are dealing with is quite large (say >100K cells), you may need to use advanced techniques like breaking up your read/writes into multiple chunks.</span></span> <span data-ttu-id="c002b-412">Следующее видео действительно для установки данных малого среднего размера.</span><span class="sxs-lookup"><span data-stu-id="c002b-412">The following video is really for a small-mid sized data setup.</span></span> <span data-ttu-id="c002b-413">Для большого количества данных обратитесь к [сценарию записи расширенных данных.](write-large-dataset.md)</span><span class="sxs-lookup"><span data-stu-id="c002b-413">For a large dataset, refer to [advanced data write scenario](write-large-dataset.md).</span></span>

<span data-ttu-id="c002b-414">[![Видео, предоставляющая совет по оптимизации чтения и записи](../../images/getting-started-v_perf.jpg)](https://youtu.be/lsR_GvVW3Pg "Совет по оптимизации чтения и записи")</span><span class="sxs-lookup"><span data-stu-id="c002b-414">[![Video providing a read-and-write optimization tip](../../images/getting-started-v_perf.jpg)](https://youtu.be/lsR_GvVW3Pg "Video showing read-and-write optimization tip")</span></span>

* <span data-ttu-id="c002b-415">`console.log` заявление (см. следующий пример)</span><span class="sxs-lookup"><span data-stu-id="c002b-415">`console.log` statement (see the following example)</span></span>
...
* <span data-ttu-id="c002b-416">`try {} catch ()` заявление</span><span class="sxs-lookup"><span data-stu-id="c002b-416">`try {} catch ()` statement</span></span>

<span data-ttu-id="c002b-417">Избегайте циклов `for` обработки исключений.</span><span class="sxs-lookup"><span data-stu-id="c002b-417">Avoid exception handling `for` loops.</span></span> <span data-ttu-id="c002b-418">Внутренние и внешние циклы.</span><span class="sxs-lookup"><span data-stu-id="c002b-418">Both inside and outside loops.</span></span>

## <a name="note-to-vba-developers"></a><span data-ttu-id="c002b-419">Примечание для разработчиков VBA</span><span class="sxs-lookup"><span data-stu-id="c002b-419">Note to VBA developers</span></span>

<span data-ttu-id="c002b-420">Язык TypeScript отличается от VBA как синтаксически, так и в соглашениях именования.</span><span class="sxs-lookup"><span data-stu-id="c002b-420">The TypeScript language differs from VBA both syntactically as well as in naming conventions.</span></span>

<span data-ttu-id="c002b-421">Ознакомьтесь со следующими эквивалентными фрагментами.</span><span class="sxs-lookup"><span data-stu-id="c002b-421">Check out the following equivalent snippets.</span></span>
...
<span data-ttu-id="c002b-422">Несколько вещей, которые нужно вызвать о TypeScript:</span><span class="sxs-lookup"><span data-stu-id="c002b-422">A few things to call out about TypeScript:</span></span>

* <span data-ttu-id="c002b-423">Вы можете заметить, что для выполнения всех методов необходимо иметь скобки с открытыми дверями.</span><span class="sxs-lookup"><span data-stu-id="c002b-423">You may notice that all methods need to have open-close parentheses to execute.</span></span> <span data-ttu-id="c002b-424">Аргументы передаются одинаково, но для выполнения могут потребоваться некоторые аргументы (то есть необходимые или необязательные).</span><span class="sxs-lookup"><span data-stu-id="c002b-424">Arguments are passed identically but some arguments may be required for execution (that is, required vs optional).</span></span>
* <span data-ttu-id="c002b-425">Конвенция именования следует camelCase вместо pascalCase convention.</span><span class="sxs-lookup"><span data-stu-id="c002b-425">The naming convention follows camelCase instead of PascalCase convention.</span></span>
* <span data-ttu-id="c002b-426">Методы обычно имеют `get` или `set` префиксы, указывающие, является ли это чтение или написание участников объекта.</span><span class="sxs-lookup"><span data-stu-id="c002b-426">Methods usually have `get` or `set` prefixes indicating whether it is reading or writing object members.</span></span>
* <span data-ttu-id="c002b-427">Блоки кода определяются и определяются фигурными скобами с открытыми дверями: `{` `}` .</span><span class="sxs-lookup"><span data-stu-id="c002b-427">The code blocks are defined and identified by open-close curly braces: `{` `}`.</span></span> <span data-ttu-id="c002b-428">Блоки необходимы для `if` условий, `while` заявлений, `for` циклов, определений функций и т.д.</span><span class="sxs-lookup"><span data-stu-id="c002b-428">Blocks are required for `if` conditions, `while` statements, `for` loops, function definitions, etc.</span></span>
* <span data-ttu-id="c002b-429">Функции могут вызывать другие функции, и вы даже можете определить функции в пределах функции.</span><span class="sxs-lookup"><span data-stu-id="c002b-429">Functions can call other functions and you can even define functions within a function.</span></span>

<span data-ttu-id="c002b-430">В целом TypeScript — это другой язык, и между ними мало общего.</span><span class="sxs-lookup"><span data-stu-id="c002b-430">Overall, TypeScript is a different language and there are few similarities between them.</span></span> <span data-ttu-id="c002b-431">Однако API сценариев Office использует аналогичную иерархию терминологии и модели данных (объектная модель) в качестве API VBA, что должно помочь вам ориентироваться.</span><span class="sxs-lookup"><span data-stu-id="c002b-431">However, the Office Scripts API themselves use similar terminology and data-model (object model) hierarchy as VBA APIs and that should help you navigate around.</span></span>
