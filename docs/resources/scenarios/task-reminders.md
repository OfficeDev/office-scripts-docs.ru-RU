---
title: 'Пример сценария сценариев Office: автоматические напоминания о задачах'
description: Пример, использующий Power Automate и адаптивные карточки, автоматизирует напоминания о задачах в электронной таблице управления проектами.
ms.date: 06/29/2021
...
ms.locfileid: "67572593"
---
# <a name="office-scripts-sample-scenario-automated-task-reminders"></a>Пример сценария сценариев Office: автоматические напоминания о задачах

В этом сценарии вы управляете проектом. Лист Excel используется для отслеживания состояния сотрудников каждый месяц. Часто необходимо напоминать людям о том, что нужно заполнить их состояние, поэтому вы решили автоматизировать этот процесс напоминания.
...
1. Сохраните скрипт с именем **Get Люди**.
...
1. Теперь необходимо создать поток. Откройте [Power Automate](https://flow.microsoft.com/).
    ...
    > [!TIP]
    > Если вы еще не создали поток, ознакомьтесь с нашим руководством. Начните использовать сценарии [с Power Automate](../../tutorials/excel-power-automate-manual.md) , чтобы изучить основные сведения.

1. Создайте новый **мгновенный поток**.

1. Выберите **вручную активировать поток из** параметров и нажмите кнопку **"Создать"**.

1. Поток должен вызвать скрипт **Get Люди**, чтобы получить всех сотрудников с пустыми полями состояния. Выберите **новый шаг**, а затем **Excel Online (бизнес)**. В разделе **Действия** выберите **Запуск скрипта**. Укажите следующие записи для шага потока:
...
1. Далее поток должен обработать каждый сотрудник в массиве, возвращаемом скриптом. Выберите **"Новый шаг**", а затем выберите **"Опубликовать адаптивную карточку" для пользователя Teams и дождитесь ответа**.

1. В поле **"Получатель** " добавьте **сообщение электронной** почты из динамического содержимого (выбранный элемент будет содержать логотип Excel). Добавление **электронной** почты приводит к тому, что шаг потока будет заключен в действие **"Применить к каждому блоку** ". Это означает, что Power Automate будет выполнять итераторную обработку массива.
...
1. В разделе **"Применить к каждому блоку** " после публикации адаптивной карточки пользователю **Teams** и ожидания ответа нажмите кнопку **"Добавить действие"**. Выберите **Excel Online (бизнес)**. В разделе **Действия** выберите **Запуск скрипта**. Укажите следующие записи для шага потока:
    ...
    :::image type="content" source="../../images/scenario-task-reminders-last-flow-step.png" alt-text="Поток Power Automate, показывающий шаг &quot;Применить к каждому&quot;.":::
...
Вы должны получить адаптивную карточку из Power Automate через Teams. После заполнения поля состояния в карточке поток продолжит работу и обно службы электронной таблицы с указанным состоянием.
...
:::image type="content" source="../../images/scenario-task-reminders-adaptive-card.png" alt-text="Адаптивная карточка в Teams, запрашивающее у сотрудника обновление состояния.":::
