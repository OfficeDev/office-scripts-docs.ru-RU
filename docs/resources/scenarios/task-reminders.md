...
16. В **пункте Применить** к каждому блоку после публикации адаптивной карты Teams пользователю и дождаться **ответа,** нажмите **кнопку Добавить действие**. Выберите **Excel Online (Бизнес).** В разделе **Действия** выберите **Запуск сценария (предварительная версия)**. Предоставление следующих записей для шага потока:
