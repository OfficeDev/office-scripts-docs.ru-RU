...
Эти кнопки сценариев также работают в Windows. Создайте кнопку в Excel в Интернете и пользователи в Windows смогут запустить ваш сценарий путем нажатия кнопки. Обратите внимание, что в Windows поддерживается только запуск кнопкой. Если вам нужно изменить сценарий, это потребуется сделать через веб-приложение.

> [!NOTE]
> Чтобы использовать эту функцию в Windows, вам потребуется копия Excel, подключенная к подписке Microsoft 365. Неподдерживаемые платформы или более ранние версии Office отображают форму, используемую для кнопки сценария, но ее нельзя нажать.
